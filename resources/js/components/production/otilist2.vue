...
<DataTable :value="customers" :paginator="true" class="p-datatable-sm" :rows="5"
	:rowHover="true"
	:rowClass="rowClass"
    ...
    <template #header>
        <div class="table-header d-flex">
            <div style="font-size: 1.8rem; width: 100%;">Nuevas ordenes</div>
    ...
    <Column field="id" headerStyle="width: 3rem; text-align: center" bodyStyle="text-align: center; overflow: visible">
        <template #body="slotProps">
            <a :href="'/adm/oti/'+ slotProps.data.id "><i class="fa fa-cog"></i></a>
        </template>
    </Column>
</DataTable>
</div>
...
export default {
    ...
    mounted() {
        this.customerService.getOtiNew().then(data => this.customers = data);
        this.loading = false;
    },
    methods: {
