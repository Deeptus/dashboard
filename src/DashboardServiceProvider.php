    ...
    public function boot()
    {
        ...
        */
        ...
        Blade::directive('hap', function ($permissions) {
            ...
            $availableForRoot = false;
            foreach ($ps as $key => $p) {
                // remove spaces
                $p = trim($p);
                $ps[$key] = $p;
                if ( $p == 'available-for-root') {
                    $availableForRoot = true;
                    unset($ps[$key]);
                } else {
            ...
            $ps = array_values($ps);
            $permissions = "'" . implode("','", $ps) . "'";
            if ( $availableForRoot ) {
                $condition = "<?php if( ( auth()->check() && auth()->user()->hasAnyPermission({$permissions}) ) || auth()->user()->root ){ ?>";
            } else {
                $condition = "<?php if( auth()->check() && auth()->user()->hasAnyPermission({$permissions}) ){ ?>";
            }
            return $condition;
        });
